...
<?php
	...
	if(isset($_POST['addusersubmit'])) {
		// get all input data
		$user_uname				= mysqli_real_escape_string($connection, $_POST['user_uname']);
		...
		$query = "SELECT cms_users.user_uname FROM cms_users 
		...
		$result = validateUserRegistration($user_uname, $user_email, $user_pass1, $user_pass2, $r);

if($result !== null) {
    $div_class = $result['div_class'];
    $div_msg = $result['div_msg'];
	echo $div_class."  ". $div_msg ;
} else { 
			...
			extract(initializeUserVariables());
		}
