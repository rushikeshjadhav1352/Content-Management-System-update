<?php
	...
	if(isset($_POST['addsubmit'])) {
		$new_cat_title = mysqli_real_escape_string($connection, $_POST['add_cat_title']);
...
<?php
	...
	if(isset($_POST['editsubmit'])) {
		$new_cat_title = mysqli_real_escape_string($connection, $_POST['edit_cat_title']);
		$edit_cat_id = $_POST['edit_cat_id'];
		
		// get categories from database to check against new one
		$q = "SELECT * FROM cms_categories";
		$cats = mysqli_query($connection, $q);
		...
		if($new_cat_title == "") {
			...
			$update_result = mysqli_query($connection, $q);
