<?php
	...
	if(isset($_POST['updateusersubmit'])) {
		...
		$user_pass1				= mysqli_real_escape_string($connection, $_POST['user_pass1']);
		$user_pass2				= mysqli_real_escape_string($connection, $_POST['user_pass2']);
		...
		switch($_POST['user_image']) {
			case "none";
				$user_image = "default.png";break;
			case "new";$user_image = $_FILES['new_image']['name'];
				break;
			default:
		...
		if(empty($user_uname) || empty($user_email) || empty($user_pass1) || empty($user_pass2)) {
			...
			$query = "update cms_users SET user_uname = '$user_uname', 
...
<?php 
	...
	if(isset($_GET['id'])) {
		...
		$query = "select * from cms_users where user_id = $edit_user_id";
		$result = mysqli_query($connection, $query);
...
<form action="" method="post" enctype="multipart/form-data">
	...
	<div class="col-md-6">
		<div class="form-group">
			...
			<!-- array of roles containing the values 'Subscriber', 'Administrator', and 'Tourist' is created -->
		...
		<div class="form-group">
			...
			<?php
				...
				// display a dropdown menu of user statuses in a form.
